        ...
        try {
            if ($transactionModel->load($post, '') && $transactionModel->validate()) {
                ...
                if (!$transactionModel->hasErrors()) {
                    ...
                    $return['status'] = true;
                    $return['msg'] = '交易成功';
                    $return['data'] = ['id' => $transactionModel->id,'use_found'=>$transactionModel->use_funds];
                } else {
                    $return['msg'] = '交易失败';
    ...
    //1. 根据交易单号，从Redis取出交易信息；
        ...
        if(!is_null($transactionStr)) {//交易单还处理生效状态
            ...
            if($transactionInfoArray['user_id'] == Yii::$app->user->id) {
                ...
                //计算盈亏，
                $profitLoss = ($currentPrice['open'] - $transactionModel->price) / $transactionConfig['basicPoint'] * $goodsItemInfo['change']; //盈亏 = （当前价格 - 交易价格）/ 交易基点  X 交易产品变动值
                ...
                $transactionModel->close_price = $currentPrice['open'];
                $transactionModel->save();
                ...
                //解冻资金
                $redis->hincrbyfloat('user:' . $transactionModel->user_id, 'freezing_funds', -$transactionModel->use_funds);
                $accountModel->freezing_funds -= $transactionModel->use_funds;
                //还原可用资金
                $redis->hincrbyfloat('user:' . $transactionModel->user_id, 'available_funds', $transactionModel->use_funds);
                ...
                $redis->hincrbyfloat('user:' . $transactionModel->user_id, 'amount', $profitLoss);//   增加资金总额
                $redis->hincrbyfloat('user:' . $transactionModel->user_id, 'available_funds', $profitLoss);//   增加可用资金
                //8. 发微信给用户，告知用户订单关闭信息；
                $redis->executeCommand('EXEC');//redis事务结束
